...
<?php include 'index.php';
	...
	//$_SESSION['from'];$_SESSION['to'];
	 ...
	 if ($_SESSION['food']=="Exported") {
      ...
      $sel=$conn->query("SELECT * from  export inner join food on(export.food_id=food.food_id) where exp_date
       between '$from' and '$to'");
...
<?php 
  ...
  if($_SESSION['food']=="Imported"){
  	$sel1=$conn->query("SELECT * from  import inner join food on(import.food_id=food.food_id) where imp_date
       between '$from' and '$to'");
...
<button onclick="document.location=''" id="pr">print</button>
